...
import com.canggo.zhishu.model.RechargePackage;
import com.canggo.zhishu.model.User;
import com.canggo.zhishu.repository.OrganizationTagRepository;
import com.canggo.zhishu.repository.RechargePackageRepository;
import com.canggo.zhishu.repository.UserRepository;
import com.canggo.zhishu.service.InviteCodeService;
import com.canggo.zhishu.service.ModelProviderConfigService;
import com.canggo.zhishu.service.RedisChatSessionStore;
import com.canggo.zhishu.service.RateLimitConfigService;
import com.canggo.zhishu.service.UsageDashboardService;
...
import com.canggo.zhishu.utils.MinioMigrationUtil;
...
public class AdminController {
    ...
    @Autowired
    private RedisChatSessionStore chatSessionStore;
    ...
    public ResponseEntity<?> getChatUsers(@RequestHeader("Authorization") String token,
                                          @RequestParam(required = false) String keyword) {
        String adminUsername = jwtUtils.extractUsernameFromToken(token.replace("Bearer ", ""));
        validateAdmin(adminUsername);

        List<User> users = userRepository.findAll().stream()
                .peek(user -> chatSessionStore.migrateLegacyConversationIfNeeded(String.valueOf(user.getId())))
                .filter(user -> keyword == null || keyword.isBlank() || user.getUsername().contains(keyword))
                .filter(user -> !chatSessionStore.listSessions(String.valueOf(user.getId())).isEmpty())
                .toList();

        List<Map<String, Object>> payload = users.stream().map(user -> {
            List<RedisChatSessionStore.ChatSessionMeta> sessions = chatSessionStore.listSessions(String.valueOf(user.getId()));
            Map<String, Object> item = new HashMap<>();
            item.put("userId", user.getId());
            item.put("username", user.getUsername());
            item.put("sessionCount", sessions.size());
            item.put("lastActiveAt", sessions.isEmpty() ? null : sessions.get(0).updatedAt());
            return item;
    ...
    @GetMapping("/chat/users/{userId}/sessions")
    public ResponseEntity<?> getUserChatSessions(@RequestHeader("Authorization") String token,
        ...
        chatSessionStore.migrateLegacyConversationIfNeeded(String.valueOf(user.getId()));
        List<RedisChatSessionStore.ChatSessionMeta> sessions = chatSessionStore.listSessions(String.valueOf(user.getId()));
        return ResponseEntity.ok(Map.of(
                "code", 200,
                "message", "获取用户会话成功",
    ...
    @GetMapping("/chat/users/{userId}/sessions/{sessionId}/messages")
    public ResponseEntity<?> getUserChatSessionMessages(@RequestHeader("Authorization") String token,
                                                        @PathVariable String userId,
        ...
        chatSessionStore.migrateLegacyConversationIfNeeded(String.valueOf(user.getId()));
        RedisChatSessionStore.ChatSessionMeta sessionMeta = chatSessionStore.requireOwnedSession(String.valueOf(user.getId()), sessionId);
        List<RedisChatSessionStore.ChatMessageRecord> messages = chatSessionStore.listMessages(sessionId, false);
    ...
    public ResponseEntity<?> getAllConversations(
        ...
        try {
            ...
            LogUtils.logBusiness("ADMIN_GET_ALL_CONVERSATIONS", adminUsername,
                    "访问已废弃的扁平聊天记录接口，目标用户ID: %s, 时间范围: %s 到 %s", userid, start_date, end_date);
            monitor.end("管理员查询对话历史接口已废弃");

            return ResponseEntity.status(HttpStatus.GONE).body(Map.of(
                    "code", HttpStatus.GONE.value(),
                    "message", "该接口已废弃，请改用 /api/v1/admin/chat/users、/api/v1/admin/chat/users/{userId}/sessions、/api/v1/admin/chat/users/{userId}/sessions/{sessionId}/messages"
            ));
        } catch (CustomException e) {
