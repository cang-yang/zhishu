...
public class OrgTagAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (path.matches(".*/upload/chunk.*") || 
                ...
                path.matches(".*/document-processing/.*") ||
                ...
                if (path.contains("/chunk")) {
                    ...
                    operation = "混合检索";
                } else if (path.contains("/document-processing/")) {
                    operation = "文档处理状态或恢复";
            ...
            // 获取资源的组织标签
            ResourceInfo resourceInfo = getResourceInfo(request, resourceId);
            ...
            // 如果是资源拥有者，直接放行
            if ((userId != null && userId.equals(resourceInfo.getOwner()))
                    || (username != null && username.equals(resourceInfo.getOwner()))) {
     ...
     */
    private ResourceInfo getResourceInfo(HttpServletRequest request, String resourceId) {
        if (resourceId == null) {
        ...
        if (ownerUserId != null && !ownerUserId.isBlank()) {
            Optional<FileUpload> exactFileUpload = fileUploadRepository.findFirstByFileMd5AndUserIdOrderByCreatedAtDesc(resourceId, ownerUserId);
            if (exactFileUpload.isPresent()) {
                FileUpload file = exactFileUpload.get();
                ResourceInfo info = new ResourceInfo(
                        file.getUserId(),
                        file.getOrgTag(),
                        file.isPublic()
                );
                logger.debug("根据 fileMd5 + ownerUserId 精确找到文件资源 => 资源ID: {}, ownerUserId: {}, 拥有者: {}, 组织标签: {}, 是否公开: {}",
                        resourceId, ownerUserId, info.getOwner(), info.getOrgTag(), info.isPublic());
                return info;
            }
        }
    ...
    private boolean isPublicResource(String resourceId) {
        Optional<FileUpload> fileUpload = fileUploadRepository.findFirstByFileMd5OrderByCreatedAtDesc(resourceId);
        return fileUpload.map(FileUpload::isPublic).orElse(false);
    }
