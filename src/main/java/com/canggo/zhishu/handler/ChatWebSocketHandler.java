package com.canggo.zhishu.handler;
...
public class ChatWebSocketHandler extends TextWebSocketHandler {
        ...
        try {
            ...
            if (payload.trim().startsWith("{")) {
                try {
                    ...
                    String requestId = asString(jsonMessage.get("requestId"));
                    String sessionId = asString(jsonMessage.get("sessionId"));
                    String messageId = asString(jsonMessage.get("messageId"));
                    String content = asString(jsonMessage.get("content"));
                    
                    if ("stop".equals(messageType) || "chat.stop".equals(messageType)) {
                        logger.info("收到停止按钮指令，用户ID: {}，会话ID: {}", userId, session.getId());
                        chatHandler.stopResponse(userId, sessionId, requestId, messageId, session);
                        return;
                    }

                    if ("chat.send".equals(messageType)) {
                        if (content == null || content.isBlank()) {
                            sendErrorMessage(session, "消息内容不能为空");
                            return;
                        }
                        chatHandler.processMessage(userId, sessionId, normalizeRequestId(requestId, session), content, session);
                        return;
                    }

                    if ("chat.regenerate".equals(messageType)) {
                        if (messageId == null || messageId.isBlank()) {
                            sendErrorMessage(session, "缺少待重生成的消息ID");
                            return;
                        }
                        chatHandler.regenerateMessage(userId, sessionId, messageId, normalizeRequestId(requestId, session), session);
                        return;
                    }
            ...
            // 普通聊天消息处理（保持向下兼容）
            chatHandler.processLegacyMessage(userId, payload, session);
    ...
    private String normalizeRequestId(String requestId, WebSocketSession session) {
