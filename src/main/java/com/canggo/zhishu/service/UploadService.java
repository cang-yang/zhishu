...
public class UploadService {
     ...
     */
    public boolean instantUploadDetermination(String fileMd5, String fileName,
                                              String orgTag, boolean isPublic, String userId) {
        //查询数据库是否存在同md5的文件且状态为
        Optional<FileUpload> fileUpload = fileUploadRepository
                .findFirstByFileMd5AndStatusOrderByCreatedAtAsc(fileMd5, 1);
        if (fileUpload.isEmpty() || fileUpload.get().getStatus() == 0) {
            return false;
        }

        logger.info("[秒传功能1]找到用户，id：{}，文件md5：{}，文件名称：{}", fileUpload.get().getUserId(), fileMd5, fileName );
        ...
        //拷贝数据库的相关数据，然后修改本次相关信息再存入数据库（指向的minio文件路径与原本相同）
        FileUpload file = new FileUpload();
        BeanUtils.copyProperties(fileUpload.get(), file);
        file.setId(null);
    ...
    public record InstantTransmissionDeterminationAttributes(long estimatedTokens, int estimatedChunkCount,
                                                             boolean instantUpload, String presignedUrl) {
    }
     ...
     * @throws IOException 如果文件读取失败
     */
    public InstantTransmissionDeterminationAttributes secondStageInstantUploadDetermination(String fileMd5, String fileName, String userId) {
        // 查询本次是否是秒传
        logger.info("[秒传功能2]开始查询是否是秒传");
        Optional<FileUpload> fileUpload = fileUploadRepository
                .findFirstByFileMd5AndUserIdOrderByCreatedAtDesc(fileMd5, userId);

        try {
            //判断是否是秒传来的
            if (fileUpload.isEmpty() || !Boolean.TRUE.equals(fileUpload.get().getIsRapidUpload())) {
                return null;
            }
    ...
    //保存向量内容到ES
    public void saveVectorData(String fileMd5, String originalUser, FileUpload fileUpload) {
        //读取es里的向量化信息并修改相关字段
        List<EsDocument> documents = elasticsearchService.queryDocument(fileMd5, originalUser);
        List<EsDocument> esDocuments = new ArrayList<>();

        // 修改 Elasticsearch 文档并存储
        for (EsDocument original : documents) {
            EsDocument copy = new EsDocument(
                    ...
                    original.getVector(),               // 向量直接复用，不需要重新生成
                    original.getModelVersion(),
                    fileUpload.getUserId(),                             // 新用户
                    fileUpload.getOrgTag(),        // 新组织
                    fileUpload.isPublic()          // 新权限
            );
            esDocuments.add(copy);
        }
        elasticsearchService.bulkIndex(esDocuments); // 批量存储到 Elasticsearch
    }

    //保存内容块到数据库
    public void saveContentBlock(String fileMd5, String originalUser, FileUpload fileUpload) {
        //获取全部文段
        List<DocumentVector> documentVectors = documentVectorRepository.findByFileMd5AndUserId(fileMd5, originalUser);
        List<DocumentVector> list = new ArrayList<>();
        //循环复制为改本次用户的
        for (DocumentVector original : documentVectors) {
            //这里不可以直接操作原数据，因为jpa会在你修改数据的时候直接同步到数据库！
            DocumentVector copy = new DocumentVector();
            copy.setFileMd5(original.getFileMd5());
            copy.setChunkId(original.getChunkId());
            copy.setTextContent(original.getTextContent());
            copy.setPageNumber(original.getPageNumber());
            copy.setAnchorText(original.getAnchorText());
            copy.setModelVersion(original.getModelVersion());
            // 设置新用户的信息
            copy.setUserId(fileUpload.getUserId());
            copy.setPublic(fileUpload.isPublic());
            copy.setOrgTag(fileUpload.getOrgTag());
            list.add(copy);
        }
        documentVectorRepository.saveAll(list);
    }
