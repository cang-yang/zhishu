...
//        第 1 步：校验注册策略 + 密码格式
//        → 你会碰到：怎么调 validateRegistrationPolicy、正则怎么用
        validateRegistrationPolicy(username, inviteCode);
        validatePassword(password);


//        第 2 步：检查用户名是否已存在
//        → 你会碰到：Optional 怎么判断、异常怎么抛
        Optional<User> usernameValidation = userRepository.findByUsername(username);
...
//        第 3 步：创建 User 对象，加密密码，第一次 save
//        → 你会碰到：PasswordUtil 怎么调、save 后 ID 是怎么生成的
        User user = new User();
        user.setUsername(username);
        user.setPassword(PasswordUtil.encode(password));
        user.setRole(User.Role.USER);
...
//        第 4 步：创建私人组织标签
//        → 你会碰到：怎么构建 OrganizationTag、外键关联怎么设
        String privateTagId = PRIVATE_TAG_PREFIX + username;
        createPrivateOrgTag(privateTagId, username, user);


//        第 5 步：设置 orgTags 和 primaryOrg，第二次 save
//        → 你会碰到：List 转逗号字符串怎么写
        user.setOrgTags(DEFAULT_ORG_TAG + "," + privateTagId);
        user.setPrimaryOrg(privateTagId);
        userRepository.save(user);
...
//        → 你会碰到：缓存服务的 API 怎么调
    ...
    public Map<String, Object> getUserOrgTags(String username) {
        ...
        // 如果缓存中没有，则从数据库获取
        if(orgTags == null || orgTags.isEmpty()|| primaryOrg == null || primaryOrg.isEmpty()) {
            User user = userRepository.findByUsername(username)
                    .orElseThrow(() -> new CustomException("User not found", HttpStatus.NOT_FOUND));
        ...
        List<OrganizationTag> tags = organizationTagRepository.findByTagIdIn(orgTags);
        if (tags != null) {
    ...
    public void setUserPrimaryOrg(String username, String primaryOrg) {
        ...
        // 检查该组织标签是否已分配给用户
        String orgTagsStr  = user.getOrgTags();
        if(orgTagsStr ==null|| orgTagsStr.isEmpty()){
            throw new CustomException("User does not have any organization tags", HttpStatus.BAD_REQUEST);
        }
        Set<String> userTags = Arrays.stream(orgTagsStr.split(",")).collect(Collectors.toSet());
        if (userTags.stream().noneMatch(tag -> tag.equalsIgnoreCase(primaryOrg))) {
