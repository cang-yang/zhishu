...
public class ParseService {
    ...
    private void checkMemoryThreshold() {
        //`Runtime.getRuntime()` 返回当前 JVM 运行时实例，通过它可以查询 JVM 的内存状态
        Runtime runtime = Runtime.getRuntime();

        //`maxMemory()` 返回 JVM 最大可用内存
        long maxMemory = runtime.maxMemory();

        //`totalMemory()` 返回 JVM 当前已经从操作系统申请到的内存。
        long totalMemory = runtime.totalMemory();

        //`freeMemory()` 返回 `totalMemory` 中还没被使用的部分。
        ...
        if (memoryUsage > maxMemoryThreshold) {
            ...
            //`System.gc()` 是**建议** JVM 执行垃圾回收。注意是"建议"不是"命令"——JVM 可以选择忽略。
            // 但在大多数情况下，JVM 会执行一次 Full GC，回收所有不再被引用的对象。
            System.gc();
