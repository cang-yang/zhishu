    ...
    >
      <template #header-extra>
        <NSpace :size="12" align="center" wrap>
          ...
          <NPopconfirm v-if="!isMobileViewport" @positive-click="handleBatchDelete">
            <template #trigger>
      ...
      <div class="min-h-0 sm:flex-1">
        <div v-if="isMobileViewport" class="invite-code-page__mobile-list">
          <article v-for="row in data" :key="row.id" class="invite-code-page__card">
            ...
            <div class="invite-code-page__meta">
              <span>使用：{{ row.usedCount }}/{{ row.maxUses }}</span>
              <span>剩余：{{ Math.max(0, row.maxUses - row.usedCount) }}</span>
              <span>创建人：{{ row.createdBy?.username || '-' }}</span>
              <span>日期：{{ dayjs(row.createdAt).format('YYYY-MM-DD') }}</span>
            </div>
          ...
          <div v-if="!data.length && !loading" class="invite-code-page__empty">暂无邀请码</div>
...
.invite-code-page__mobile-list {
  display: flex;
  flex-direction: column;
  gap: 12px;
}

.invite-code-page__card {
  display: flex;
  flex-direction: column;
  gap: 12px;
...
.invite-code-page__card-header,
.invite-code-page__status-row,
.invite-code-page__quick-actions {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}
...
.invite-code-page__code {
  font-family: Consolas, 'Courier New', monospace;
  font-size: 14px;
  font-weight: 700;
}

.invite-code-page__meta {
  display: grid;
  gap: 6px;
