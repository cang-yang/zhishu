...
import { computed, onActivated, onBeforeUnmount, onMounted, ref, h } from 'vue';
...
function normalizePackages(payload: unknown) {
  if (Array.isArray(payload)) return payload as Api.Recharge.Package[];

  const data = payload as { records?: Api.Recharge.Package[]; list?: Api.Recharge.Package[]; content?: Api.Recharge.Package[] } | null;
...
const getPackages = async () => {
  ...
  try {
    ...
    let nextPackages = !error ? normalizePackages(data) : [];

    if (!nextPackages.length) {
      const { error: fallbackError, data: fallbackData } = await request<Api.Recharge.Package[]>({
        url: '/admin/recharge-packages',
        method: 'get'
      });

      if (!fallbackError) {
        nextPackages = normalizePackages(fallbackData);
      }
    }

    packages.value = nextPackages.filter(item => item.enabled !== false && item.deleted !== true);
  } finally {
    loading.value = false;
  }
};
...
<template>
  <div class="admin-page-shell min-h-500px flex-col-stretch gap-16px <sm:overflow-auto">
    ...
    <!-- 余额充值区域 -->
      ...
      <NSpin :show="loading">
        <div v-if="packages.length > 0" class="grid gap-6" :class="isMobileViewport ? 'grid-cols-1' : 'md:grid-cols-2 lg:grid-cols-3'">
          <!-- 充值套餐卡片 -->
          <NCard
    ...
    <NCard title="充值记录" :bordered="false" size="large" class="w-full card-wrapper">
      ...
      <NSpin :show="orderListLoading">
        <div v-if="isMobileViewport && orders.length > 0" class="recharge-page__order-list mt-4">
          <article v-for="row in orders" :key="row.tradeNo" class="recharge-page__order-card">
            <strong>{{ row.description || '充值订单' }}</strong>
            <span>订单号：{{ row.tradeNo }}</span>
            <span>金额：¥{{ (row.amount / 100).toFixed(2) }}</span>
            <span>LLM：{{ formatTokenWan(row.llmToken) }}</span>
            <span>Embedding：{{ formatTokenWan(row.embeddingToken) }}</span>
            <div class="flex items-center gap-8px flex-wrap">
              <NTag :type="({ NOT_PAY: 'default', PAYING: 'warning', SUCCEED: 'success', FAIL: 'error', CANCELLED: 'default' } as any)[row.status] || 'default'">
                {{ ({ NOT_PAY: '未支付', PAYING: '支付中', SUCCEED: '支付成功', FAIL: '支付失败', CANCELLED: '已取消' } as any)[row.status] || '未知' }}
              </NTag>
            </div>
            <span>支付时间：{{ row.payTime ? new Date(row.payTime).toLocaleString('zh-CN') : '-' }}</span>
            <span>创建时间：{{ new Date(row.createdAt).toLocaleString('zh-CN') }}</span>
          </article>
        </div>
        <NDataTable
...
.recharge-page__order-list {
  display: flex;
  flex-direction: column;
  gap: 12px;
}

.recharge-page__order-card {
  display: flex;
  flex-direction: column;
  gap: 8px;
...
.recharge-page__order-card strong {
  color: var(--app-text-primary);
  font-size: 15px;
}
