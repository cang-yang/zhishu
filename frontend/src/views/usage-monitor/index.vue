    ...
    <NCard :bordered="false" size="small" class="card-wrapper">
      ...
      <NSpin :show="overviewLoading">
        <div class="flex flex-col gap-4">
          <div class="grid gap-4" :class="isMobileViewport ? 'grid-cols-2' : 'xl:grid-cols-5 sm:grid-cols-2'">
...
.summary-card.is-alert {
  ...
  background: linear-gradient(180deg, rgba(120, 53, 15, 0.5), rgba(68, 32, 6, 0.82));
}
...
.summary-value {
  @apply mt-3 text-7 font-semibold;
  color: var(--color-text-primary);
}
...
.ranking-index {
  @apply inline-flex h-8 w-8 items-center justify-center rounded-full text-xs font-semibold text-white;
  background: linear-gradient(135deg, rgba(59, 130, 246, 0.82), rgba(6, 182, 212, 0.82));
}
