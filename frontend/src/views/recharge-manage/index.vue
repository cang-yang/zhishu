...
<template>
  <div class="admin-page-shell min-h-500px flex-col-stretch gap-16px overflow-hidden <sm:overflow-auto">
    <NCard title="充值套餐管理" :bordered="false" size="small" class="card-wrapper">
      ...
      <div v-if="isMobileViewport" class="recharge-manage-page__mobile-list mt-4">
        <article v-for="item in data" :key="item.id" class="recharge-manage-page__card">
          <strong>#{{ item.sortOrder }} · ID: {{ item.id }}</strong>
          <span>套餐名称：{{ item.packageName }}</span>
          <span>价格：¥{{ (item.packagePrice / 100).toFixed(2) }}</span>
          <span>LLM：{{ formatTokenWan(item.llmToken) }}</span>
          <span>Embedding：{{ formatTokenWan(item.embeddingToken) }}</span>
          <div class="flex items-center gap-8px flex-wrap">
            <NTag :type="item.enabled ? 'success' : 'default'">{{ item.enabled ? '已启用' : '已禁用' }}</NTag>
          </div>
          <span>创建：{{ dayjs(item.createdAt).format('YYYY-MM-DD HH:mm:ss') }}</span>
...
.recharge-manage-page__mobile-list {
  display: flex;
  flex-direction: column;
  gap: 12px;
}

.recharge-manage-page__card {
  display: flex;
  flex-direction: column;
  gap: 8px;
...
.recharge-manage-page__card strong {
  color: var(--app-text-primary);
  font-size: 15px;
}
</style>
